const toggle=document.querySelector('.menu-toggle');const nav=document.querySelector('.nav');if(toggle&&nav){toggle.addEventListener('click',()=>nav.classList.toggle('open'));nav.querySelectorAll('a').forEach(a=>a.addEventListener('click',()=>nav.classList.remove('open')))}const wa=document.getElementById('whatsapp');if(wa){wa.addEventListener('click',()=>{const nom=document.getElementById('nom').value.trim();const tel=document.getElementById('tel').value.trim();const service=document.getElementById('service').value;const msg=document.getElementById('message').value.trim();const text=`Bonjour, je souhaite demander un devis.\nNom : ${nom}\nTéléphone : ${tel}\nService : ${service}\nBesoin : ${msg}`;window.open(`https://wa.me/33605587173?text=${encodeURIComponent(text)}`,'_blank')})}
